...
### Input Specification:
Each input file contains one test case. For each case, the first line contains a positive integer N (¡Ü1000). Then N integer keys are given in the next line. All the numbers in a line are separated by a space.
...
```
### Solution
Check recursively. Oh hu~
Actually the check part can be easier.
```C++
#include<iostream>
#include<vector>
#include<cmath>
...
{
    postOrder.push_back(preOrder[left]);  //Push the current number opposite-postorderly
    if (left == right)
    {
        return true;
    }
    ...
    if (greater > right)
    {
        //Check the right part fisrt then the left part to make an opposite order of postOrder
