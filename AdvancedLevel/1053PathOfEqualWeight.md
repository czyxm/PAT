...
![](../img/1053.jpg)
### Input Specification:
Each input file contains one test case. Each case starts with a line containing 0<N¡Ü100, the number of nodes in a tree, M (<N), the number of non-leaf nodes, and 0<S<2^30, the given weight number. The next line contains N positive numbers where Wi (<1000) corresponds to the tree node Ti . Then M lines follow, each in the format:
```
ID K ID[1] ID[2] ... ID[K]
```
...
Note: sequence {A1,A2,...,An} is said to be greater than sequence {B1,B2,...,Bm} if there exists 1 <= k < min{n,m} such that Ai=Bi for i=1,...,k and Ak+1>B?k+1.
### Sample Input:
```
20 9 24
